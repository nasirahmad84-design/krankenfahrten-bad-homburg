...
- Eine freiwillig aktivierbare Adresshilfe fragt ab vier Zeichen höchstens fünf deutsche Treffer beim öffentlichen Photon-Dienst ab. Die Anfrage erfolgt ohne Browser-Credentials und Referrer; bei Ablehnung, Ausfall oder fehlendem JavaScript bleiben Abhol- und Zieladresse frei ausfüllbar. Die Oberfläche nennt Photon und OpenStreetMap unmittelbar an der Funktion.
...
Inter wird während des Next-Builds lokal eingebettet. Bilder, SVGs, Styles und die übrigen Anwendungsskripte werden von derselben Domain ausgeliefert; es bestehen keine externen Font-CDNs, Karten, Videos oder Social-Media-Widgets. Nur nach Analytics-Einwilligung wird das Google-Tag von `www.googletagmanager.com` geladen und werden Messdaten an Google-Analytics-Endpunkte übertragen. Der öffentliche Photon-Dienst wird erst nach gesonderter Aktivierung der optionalen Adressvorschläge angesprochen; manuelle Eingabe bleibt der ausfallsichere Standard. Telefon-, WhatsApp- und Social-Links öffnen externe Ziele erst durch eine bewusste Nutzeraktion.
...
Die ALL-INKL-Schritte stehen in `deployment/ALL-INKL.md`; ergänzend existieren die PHP-Abnahmematrix, Rollback-Anleitung sowie Deployment-, Go-live- und Legal-Review-Checklisten. Das vollständige Upload-Paket liegt nach `npm run build` unter `out/`. **Auf dem Zielserver ist kein Node.js-Prozess erforderlich.** Ein vollständiger Produktionsrelease wird ausschließlich manuell über `.github/workflows/deploy-live-manual.yml` ausgelöst. Der Workflow verwendet die geschützten GitHub-Zugangsdaten, prüft die serverseitige `api/config.php`, baut und verifiziert das gesamte Paket und überschreibt das produktive Secret nicht. Wiederkehrende, bereits freigegebene Ratgeberartikel werden davon getrennt als begrenztes Blog-Delta veröffentlicht.
...
Das Cockpit wird ausschließlich über den manuellen Workflow `.github/workflows/editorial-cockpit-test.yml` nach `https://test.krankenfahrten-bad-homburg.de/redaktion/` übertragen. Der Zugang erfolgt ohne KAS-Verzeichnisschutz per Einmalcode an das fest hinterlegte interne Postfach. Es gibt keine Datenbank und keine Freigabe-API; das notwendige PHP-Session-Cookie ist auf `/redaktion/` beschränkt, `Secure`, `HttpOnly` und `SameSite=Strict`. Die verbindliche Freigabe wird weiterhin erst nach ausdrücklicher Betreiberentscheidung im Repository versioniert. Die Einrichtung ist in `deployment/editorial-cockpit.md` beschrieben.
