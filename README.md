...
Der rechnerunabhängige Queue-Publisher liegt in `.github/workflows/blog-manual.yml`. Er plant montags und donnerstags ausschließlich bereits redaktionell vorbereitete und ausdrücklich freigegebene Beiträge ein. Zum Veröffentlichungszeitpunkt findet kein bezahlter KI-Aufruf statt. Der Workflow bleibt bis zur Betreiberfreigabe des aktuellen Batches deaktiviert; die Live-Übertragung besitzt zusätzlich das Gate `BLOG_QUEUE_LIVE_ENABLED`. Facebook ist noch nicht automatisiert.

## Geschütztes Redaktionscockpit

`npm run editorial:build` erzeugt aus den vorbereiteten Artikelläufen einen vollständig separaten internen Export unter `out-editorial/`. Die Übersicht und acht Artikelseiten stellen Artikeltext, Termine, Status, Quellen, Recherchebrief, Claim-Prüfung und Facebook-Entwurf lesbar dar. `npm run editorial:verify` prüft Vollständigkeit, H1, `noindex`, Secret-Ausschluss und die Trennung vom öffentlichen `out/`-Paket.

Das Cockpit wird ausschließlich über den manuellen Workflow `.github/workflows/editorial-cockpit-test.yml` nach `https://test.krankenfahrten-bad-homburg.de/redaktion/` übertragen. `scripts/deploy-editorial-ftp.sh` bricht ab, wenn dieses Verzeichnis vor dem Upload nicht bereits per HTTP Basic Auth geschützt ist. Es gibt keine Datenbank, keine Cookies, keine Freigabe-API und keinen Upload auf die Hauptdomain. Die verbindliche Freigabe wird weiterhin erst nach ausdrücklicher Betreiberentscheidung im Repository versioniert. Die Einrichtung ist in `deployment/editorial-cockpit.md` beschrieben.
